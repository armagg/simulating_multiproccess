...
# f is the vector field function: arraydot=f(array), array is the vector (x,y,z)
def RungeKutta(f, arr, t, func_n):
...
def simulate(x0, y0, z0, func_n):
    ...
    while(check == 1):
        ...
        save = np.transpose(
            np.array([t_save, x_save, y_save, z_save, z_fix, difference]))
        df = pd.DataFrame(
            save, columns=['time', 'x', 'y', 'z', 'z_fixed', 'difference'])
        df.to_csv('(x0,y0,z0)=(%.2f,%.2f,%.2f),eq%s.csv' % (
            x0, y0, z0, const.funcylabel[func_n]), mode='a', header=(i == 0), index=False)
    ...
    df2.to_csv('results/(x0,y0,z0)=(%.2f,%.2f,%.2f),eq%s_check.csv' %
               (x0, y0, z0, const.funcylabel[func_n]), mode='a', header=True)
